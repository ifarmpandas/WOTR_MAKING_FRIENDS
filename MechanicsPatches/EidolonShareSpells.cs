...
namespace WOTR_MAKING_FRIENDS.MechanicsPatches
{
    ...
    public class EidolonShareSpellsPatch
    {
        ...
        static bool CanTarget_patch(AbilityData __instance, TargetWrapper target, ref bool __result)
        {
            ...
            {
                if (Guid.Parse(__instance.SpellbookBlueprint.AssetGuidThreadSafe) != Guid.Parse(GetGUID.GUIDByName("SummonerSpellbookSpellBook")))
                {
                    return true;
                }
        ...
        {
            // add modmenu settings check
            if (__instance.Range == AbilityRange.Personal && (__instance.Blueprint.Type == AbilityType.Spell || __instance.Blueprint.Type == AbilityType.SpellLike))
            {
